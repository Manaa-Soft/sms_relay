frappe.ui.form.on('SMS Template', {
    ...
    language(frm) {
        if (frm.doc.language) {
            frappe.db.get_value('Language', frm.doc.language, 'language_code', function(r) {
                if (r && r.language_code) {
                    frm.set_value('language_code', r.language_code);
                }
            });
        } else {
            frm.set_value('language_code', '');
        }
    },

    template_name(frm) {
        if (frm.doc.template_name && !frm.doc.actual_name) {
            frm.set_value('actual_name', frm.doc.template_name);
        }
    }
});
